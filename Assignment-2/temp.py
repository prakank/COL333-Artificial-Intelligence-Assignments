...
def arc_valid(X, Xi, Xj, param):
    ...
    if Xi.row == Xj.row:
        ...
        if (Xj.row == param["N"]-1):
            ...
            if (param["m"] - count["m"]) > available:
                return False
            if (param["e"] - count["e"]) > available:
                return False
            if (param["a"] - count["a"]) > available:
                return False
            return True
    else:
        print("Error")
...
def AC_3(X, arcs, Dx, param):
    ...
    while(len(arcs) > 0):
        ...
        if (revised == True):
            #print(Dx)
            if (np.count_nonzero(np.array(Dx[Xi.row][Xi.col]) == '.') == 4):
                print("tes")
                return (False, Dx)
...
def back_track(row, col, param, Dx, X):    
    ...
    #        print(Dx)
    global count
    if (count %50000 == 0) : print("Count: {}, Row: {}, Col: {}".format(count, row, col))
    count+=1
...
def random():
    ...
    ["M", "R", "M", "R", "M", "R"],
    ["E", ".", ".", ".", ".", "."]]
    X = np.array([np.array([None for _ in range(6)]) for __ in range(5)])
    for r in range(5):
        for c in range(6):
            X[r][c] = Node(r,c)
            X[r][c].value = M[r][c]
    ...
    for r in range(5):
        for c in range(6):
            if (r >= 1):
                print(r, c, "a", arc_valid(X, X[r-1][c], X[r][c], param))
            if (c >= 1):
                print(r, c, "b", arc_valid(X, X[r][c-1], X[r][c], param))
